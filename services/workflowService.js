...
class WorkflowService {
  ...
  static async generateApprovalWorkflow(requestorId, workflowType = 'general', options = {}) {
    try {
      ...
      // Step 5b: Add CEO as absolute final approver for purchase/budget workflows
      if (workflowType === 'purchase' || workflowType === 'budget') {
        ...
        if (ceo) {
          ...
          if (!seenApprovers.has(email)) {
            ...
            seenApprovers.get(email).capacities.push('ceo');
          }
        }
      }
   ...
   * Priority: ceo > direct_supervisor > department_head > business_head > finance_officer
   */
  static getPrimaryCapacity(capacities) {
    const priority = [
      'ceo',                       // ← CEO is highest priority
      'direct_supervisor',
      'department_head',
      'technical_director',
      'business_head',
