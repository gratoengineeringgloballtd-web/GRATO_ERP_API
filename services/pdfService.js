...
class PDFService {
  ...
  // ============================================================
  // drawItemsTable — THE ONLY METHOD THAT CHANGED vs old version
  // taxRate stored as percent (e.g. 5.5 or 19.25) → convert /100
  // ============================================================
  ...
  //   // ─────────────────────────────────────────────────────────────────────────
  //   // FIX: taxRate is stored as a PERCENTAGE (e.g. 5.5 or 19.25).
  //   //      Divide by 100 to get the decimal multiplier for calculations.
  //   //      Guard: if someone ever stores it already as a decimal (< 1), keep it.
  //   // ─────────────────────────────────────────────────────────────────────────
  //   let taxRate = 0;
  ...
  //     console.log(`Item ${index}: subtotal=${itemSubtotal} tax=${taxAmount} total=${itemTotal}`);
  ...
  //     // Display the human-readable percentage (e.g. "5.50% G" or "19.25% G")
  ...
  //   // grandTotal is tax-inclusive; pass taxRate decimal to drawOrderSummary
  ...
  // ============================================================
// drawItemsTable — taxRate stored as percent (e.g. 5.5)
// Tax is calculated ON the gross: x = taxRate*x + net
// → gross = net / (1 - taxRate)
// ============================================================
// drawItemsTable(doc, yPos, poData, currentPage) {
//   console.log('=== DRAWING ITEMS TABLE ===');
...
//   // taxRate stored as a PERCENTAGE (e.g. 5.5).
//   // Tax is levied on the gross (inclusive method):
//   //   gross = net / (1 - taxRate_decimal)
//   // Guard: if already a decimal (< 1), keep it.
//   let taxRate = 0;
//   if (poData.taxApplicable) {
//     const rawRate = typeof poData.taxRate === 'number' ? poData.taxRate : 19.25;
//     taxRate = rawRate > 1 ? rawRate / 100 : rawRate;
...
//     const itemSubtotal   = quantity * unitPrice;           // net before discount
//     const discountAmount = itemSubtotal * (discount / 100);
//     const afterDiscount  = itemSubtotal - discountAmount;  // net after discount

//     // Gross-inclusive tax: gross = net / (1 - taxRate)
...
//   // grandTotal is gross-inclusive; pass taxRate decimal to drawOrderSummary
//   this.drawOrderSummary(doc, currentY, grandTotal, taxRate);
//   currentY += 90;
...
drawItemsTable(doc, yPos, poData, currentPage) {
  ...
  // ============================================================
  // drawOrderSummary — receives grandTotal (tax-inclusive) and
  // taxRate as a DECIMAL (e.g. 0.055).  Back-calculates net/vat.
  // ============================================================
  ...
  //   // grandTotal = netAmount * (1 + taxRate)  →  netAmount = grandTotal / (1 + taxRate)
  ...
  //   // VAT row — label shows actual % (e.g. "VAT 5.50%")
...
//         // NOTE: This loop uses resolveSignaturePath directly (not renderSignatureImage)
//         // because generateTenderApprovalFormPDF is not async-aware in its Promise executor.
//         // To fully patch, convert the outer Promise to async or extract the loop to an async method.
//         for (let i = 0; i < SIG_TABLE_ROWS; i++) {
//           const step      = approvalChain[i];
//           const isApproved= step && step.status === 'approved';
//           const fb        = fallbackRows[i] || {};
...
//     // Signature Section (immediately after payment terms) — fire-and-forget async
//     this.drawSignatureSection(doc, yPos, poData);
...
//     if (poData.taxApplicable) {
//       const rawRate = typeof poData.taxRate === 'number' ? poData.taxRate : 19.25;
//       // taxRate is stored as a percentage (e.g. 5.5 or 19.25), convert to decimal
//       taxRate = rawRate > 1 ? rawRate / 100 : rawRate;
//       console.log(`Tax is applicable, using ${(taxRate * 100).toFixed(2)}%`);
//     }
...
//   // ── drawSignatureSection() ── now async ──────────────────────────────────────
//   async drawSignatureSection(doc, yPos, poData) {
//     const defaultSignatures = [
//       { label: 'Supply Chain' },
//       { label: 'Finance' },
...
//   // ── drawRequesterAcknowledgmentSignature() — now async ──────────────────────
//   async drawRequesterAcknowledgmentSignature(doc, yPos, data) {
//     yPos += 20;
...
//   // ── drawBuyerAcknowledgmentSignature() — now async ──────────────────────────
//   async drawBuyerAcknowledgmentSignature(doc, yPos, data) {
//     yPos += 20;
...
//   // ── drawApproverSignatures() — now async ────────────────────────────────────
//   async drawApproverSignatures(doc, yPos, data) {
//     yPos += 5;
...
//     let taxRate = 0;
//     if (poData.taxApplicable) {
//       taxRate = typeof poData.taxRate === 'number'
//         ? poData.taxRate / 100
//         : 0.1925;
//       console.log(`Tax is applicable, using ${taxRate * 100}%`);
//     }
