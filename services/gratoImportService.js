...
// Auto-populate GeneratorAssignmentLedger on every GRATO upload so the
// ledger never needs to be manually backfilled after the first run.
const generatorSwapService = require('./generatorSwapService');
...
async function updateSiteFromVisit(v, techUserId, uploadedById) {
  ...
  if (fd.qte_trouvee    != null) {
    $set.Fuel_Quantity_Found    = fd.qte_trouvee;
    $set.Fuel_Level_Source      = 'field_visit';
    $set.Fuel_Level_Updated_At  = v.visit_date || new Date();
  }
...
async function importGratoExcel(fileBuffer, cycleKey, uploadedById) {
  ...
  for (const v of visits) {
    try {
      ...
      // Auto-maintain GeneratorAssignmentLedger so the reconciliation
      // engine always has generator data without needing a manual backfill.
      // Best-effort via setImmediate — never blocks the import loop.
      const gen1 = v.generators_checked[0];
      if (gen1?.brand || gen1?.kva) {
        setImmediate(async () => {
          try {
            const activeEntry = await GeneratorAssignmentLedger.findOne({
              site_id: v.ihs_id,
              is_active: true,
            }).select('generator_id').lean();

            const incomingId = gen1.serial_number
              || `UNKNOWN_${v.ihs_id}_${v.visit_date.getTime()}`;

            if (!activeEntry) {
              // No entry at all — assign for the first time
              await generatorSwapService.assignGenerator({
                site_id:       v.ihs_id,
                site_name:     v.site_name,
                cluster:       v.cluster,
                region:        v.region,
                generator_id:  incomingId,
                generator_brand: gen1.brand || null,
                dg_kva:        gen1.kva     || null,
                assigned_at:   v.visit_date,
                assigned_reason: 'Auto-assigned from GRATO upload',
                recorded_by:   uploadedById,
              });
            } else if (
              gen1.serial_number &&
              activeEntry.generator_id !== gen1.serial_number &&
              !activeEntry.generator_id.startsWith('UNKNOWN_')
            ) {
              // Real serial number changed — record a swap
              await generatorSwapService.swapGenerator({
                site_id:         v.ihs_id,
                site_name:       v.site_name,
                cluster:         v.cluster,
                region:          v.region,
                old_generator_id: activeEntry.generator_id,
                new_generator_id: gen1.serial_number,
                new_generator_brand: gen1.brand || null,
                new_dg_kva:      gen1.kva || null,
                swap_date:       v.visit_date,
                swap_reason:     'Detected via GRATO upload — serial number changed',
                recorded_by:     uploadedById,
              });
            }
            // If serial matches (or both are UNKNOWN), nothing to do —
            // the existing active entry is still correct.
          } catch (ledgerErr) {
            logger.error(`[GRATO] GeneratorLedger auto-update failed for ${v.ihs_id}: ${ledgerErr.message}`);
          }
        });
      }
