...
const PurchaseOrderSchema = new mongoose.Schema({
  ...
  status: {
    ...
    enum: [
      ...
      'pending_ceo_approval',        // ← NEW: high-value POs above CEO threshold
      'approved',
      'sent_to_supplier',
      'acknowledged',
      'in_production',
  ...
  // ── Assignment fields (used by assignBySupplyChain method + controller) ──
  assignedDepartment: {
    type: String,
    default: null
  },
  ...
  // ── Rejection details (set by rejectPO controller) ────────────────────────
  rejectionDetails: {
    rejectedBy: {
      type: mongoose.Schema.Types.ObjectId,
      ref: 'User'
    ...
    rejectedAt: Date,
    reason: String,
    stage: String   // e.g. 'supply_chain', 'level_1', 'level_2', 'level_3'
  },
  ...
  activities: [{
    type: {
      type: String,
      enum: ['created', 'sent', 'acknowledged', 'updated', 'shipped',
             'delivered', 'cancelled', 'approved', 'rejected'],
      required: true
    },
    description: {
      type: String,
  ...
  integrationData: {
    ...
    itemsFromDatabase: { type: Number, default: 0 },
    manualItems: { type: Number, default: 0 },
    supplierValidated: { type: Boolean, default: false },
    itemsValidated: { type: Boolean, default: false }
  },
...
PurchaseOrderSchema.index({ tenderId: 1 });
PurchaseOrderSchema.index({ createdWithoutTender: 1 });
PurchaseOrderSchema.index({ assignedDepartment: 1 });
PurchaseOrderSchema.index({ 'supplyChainReview.reviewedBy': 1 });
PurchaseOrderSchema.index({ 'approvalChain.approver.email': 1 });
...
PurchaseOrderSchema.virtual('itemsBreakdown').get(function () {
  ...
  return {
    ...
    fromDatabase,
    manual,
    databasePercentage: this.items.length > 0
      ? Math.round((fromDatabase / this.items.length) * 100)
      : 0
  };
});
...
PurchaseOrderSchema.virtual('approvalProgress').get(function () {
  ...
  const approved = this.approvalChain.filter(s => s.status === 'approved').length;
  return Math.round((approved / this.approvalChain.length) * 100);
});
...
PurchaseOrderSchema.methods.addActivity = function (type, description, user) {
  this.activities.push({ type, description, user: user || 'System', timestamp: new Date() });
  return this.save();
};
...
PurchaseOrderSchema.methods.updateStatus = function (newStatus, user) {
  ...
  const progressMap = {
    draft: 5, pending_supply_chain_assignment: 8,
    pending_department_approval: 15, pending_head_of_business_approval: 20,
    pending_finance_approval: 25, pending_ceo_approval: 28,
    approved: 35, sent_to_supplier: 45, acknowledged: 55,
    in_production: 65, ready_for_shipment: 75, in_transit: 85,
    delivered: 95, completed: 100, cancelled: 0, on_hold: this.progress
  };
  this.progress = progressMap[newStatus] ?? this.progress;

  const stageMap = {
    draft: 'created', pending_supply_chain_assignment: 'created',
    pending_department_approval: 'created', pending_head_of_business_approval: 'created',
    pending_finance_approval: 'created', pending_ceo_approval: 'created',
    approved: 'created', sent_to_supplier: 'supplier_acknowledgment',
    acknowledged: 'supplier_acknowledgment', in_production: 'in_production',
    ready_for_shipment: 'in_production', in_transit: 'in_transit',
  ...
  try {
    ...
    for (const item of this.items) {
      if (item.itemId) {
        ...
        if (!dbItem) {
          results.valid = false;
          results.errors.push(`Item ${item.description}: not found in database`);
        } else if (!dbItem.isActive) {
          results.valid = false;
          results.errors.push(`Item ${item.description}: inactive`);
        } else {
    ...
    this.integrationData.itemsValidated = results.valid;
    return results;
  } catch (error) {
    console.error('Error validating items:', error);
    return { valid: false, errors: ['Error validating items'], warnings: [] };
  }
};
...
PurchaseOrderSchema.methods.calculateDeliveryPerformance = function () {
  if (this.actualDeliveryDate && this.expectedDeliveryDate) {
    const diffDays = Math.ceil(
      (new Date(this.actualDeliveryDate) - new Date(this.expectedDeliveryDate)) / 86400000
    );
    ...
    return { onTime: diffDays <= 0, variance: diffDays, status: diffDays <= 0 ? 'On Time' : `${diffDays} days late` };
  }
  return null;
};

PurchaseOrderSchema.methods.getCurrentApprover = function () {
  if (!this.currentApprovalLevel || !this.approvalChain) return null;
  return this.approvalChain.find(s => s.level === this.currentApprovalLevel);
};

PurchaseOrderSchema.methods.canUserApprove = function (userEmail) {
  const step = this.getCurrentApprover();
  return step && step.approver.email === userEmail;
};

PurchaseOrderSchema.methods.getApprovalHistory = function () {
  if (!this.approvalChain) return [];
  return this.approvalChain.filter(s => s.status !== 'pending').sort((a, b) => a.level - b.level);
};
...
PurchaseOrderSchema.methods.assignBySupplyChain = function (department, assignedByUserId, comments) {
  ...
  if (!['draft', 'pending_supply_chain_assignment'].includes(this.status)) {
    throw new Error('PO is not in a valid status for assignment');
  }
  ...
  const chain = getPOApprovalChain(department, this.totalAmount);
  if (!chain || chain.length < 3) {
    throw new Error(`Failed to create approval chain for ${department}`);
  }
...
PurchaseOrderSchema.methods.rejectBySupplyChain = function (rejectedByUserId, rejectionReason) {
  ...
  this.supplyChainReview = {
    reviewedBy: rejectedByUserId, reviewDate: new Date(),
    reviewTime: new Date().toTimeString().split(' ')[0],
    action: 'rejected', rejectionReason
  };
  this.rejectionDetails = {
    rejectedBy: rejectedByUserId, rejectedAt: new Date(),
    reason: rejectionReason, stage: 'supply_chain'
  };
};

PurchaseOrderSchema.methods.processApprovalStep = function (approverEmail, decision, comments, userId) {
  const currentStep = this.approvalChain.find(
    s => s.level === this.currentApprovalLevel && s.approver.email === approverEmail && s.status === 'pending'
  );
  if (!currentStep) throw new Error(`Not authorized to approve at level ${this.currentApprovalLevel}`);
  ...
  if (decision === 'rejected') {
    ...
    this.rejectionDetails = {
      ...
      reason: comments, stage: `level_${this.currentApprovalLevel}`
    };
    this.currentApprovalLevel = 0;
  } else {
    const nextLevel = this.currentApprovalLevel + 1;
    const nextStep  = this.approvalChain.find(s => s.level === nextLevel);
    ...
    if (nextStep) {
      ...
      // Map level to status — supports 3-level (dept/hob/finance) and
      // 4-level (dept/hob/finance/ceo) chains
      const statusByLevel = {
        2: 'pending_head_of_business_approval',
        3: 'pending_finance_approval',
        4: 'pending_ceo_approval'
      };
      this.status = statusByLevel[nextLevel] || 'pending_department_approval';
    } else {
          ...
          <div style="background-color:#fff3cd;padding:20px;border-radius:8px;border-left:4px solid #ffc107;">
            ...
            <div style="background-color:white;padding:20px;border-radius:8px;margin:20px 0;">
              ...
              <table style="width:100%;">
                ...
                <tr><td><strong>Supplier:</strong></td><td>${this.supplierDetails?.name || ''}</td></tr>
...
PurchaseOrderSchema.statics.getByBuyer = function (buyerId, options = {}) {
  ...
  return this.find(query)
    ...
    .populate('supplierId', 'name email phone address performance')
...
PurchaseOrderSchema.statics.getBuyerStats = async function (buyerId) {
  const stats = await this.aggregate([
    { $match: { buyerId: new mongoose.Types.ObjectId(buyerId) } },
    {
      $group: {
        ...
        active: { $sum: { $cond: [{ $in: ['$status', ['approved', 'sent_to_supplier', 'acknowledged', 'in_production', 'in_transit']] }, 1, 0] } },
        completed:  { $sum: { $cond: [{ $eq: ['$status', 'completed'] }, 1, 0] } },
        cancelled:  { $sum: { $cond: [{ $eq: ['$status', 'cancelled'] }, 1, 0] } },
        avgDeliveryTime:      { $avg: '$performanceMetrics.deliveryVariance' },
        onTimeDeliveries:     { $sum: { $cond: [{ $eq: ['$performanceMetrics.onTimeDelivery', true] }, 1, 0] } },
  ...
  return stats[0] || {
    total: 0, totalValue: 0, active: 0, completed: 0, cancelled: 0,
    avgDeliveryTime: 0, onTimeDeliveries: 0, totalItemsFromDatabase: 0, totalManualItems: 0
  };
};

PurchaseOrderSchema.statics.getPendingSupplyChainAssignment = function () {
  return this.find({ status: 'pending_supply_chain_assignment' })
    .populate('buyerId', 'fullName email')
...
// ── Post-save: trigger approval notifications ─────────────────────────────────
PurchaseOrderSchema.post('save', async function () {
  ...
  if (approvableStatuses.includes(this.status) && this.currentApprovalLevel > 0 && this.approvalChain.length > 0) {
    const currentStep = this.getCurrentApprover();
