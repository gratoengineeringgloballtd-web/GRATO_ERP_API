...
mongoose.connect(process.env.MONGO_URI).then(async () => {
  try {
    // Find the budget code - Operations Opex 2026
    const budgetCode = await BC.findOne({
      $or: [
        { code: /OPR.*OPEX/i },
        { code: /OPERATIONS.*OPEX/i },
        { name: /OPERATIONS.*OPEX/i }
      ]
    });

    // Find the requisition
    const requisition = await PR.findOne({ requisitionNumber: 'REQ202601058500' });
    ...
    if (requisition) {
      console.log('✓ Requisition Found: REQ202601058500');
    ...
    console.log('  Status Updated: pending_finance_verification → pending_supply_chain_review');
