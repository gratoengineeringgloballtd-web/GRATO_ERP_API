...
/**
 * FIXED: Get IT support approval chain with CEO as absolute final step
 * Chain: Supervisor → Dept Head → HR & Admin Head → IT Department → CEO
 *
 * @param {string} employeeEmail - Email of employee requesting IT support
 * @returns {array} - Approval chain with CEO always as final step
 */
const getITSupportApprovalChain = (employeeEmail) => {
  ...
  if (hasIT) {
    ...
    // Add CEO AFTER IT — absolute final
    mappedChain.push({
      level:        mappedChain.length + 1,
      approver: {
        name:       'Mr. Tom',
    ...
    console.log('✓ Appending HR & Admin Head, IT Department, and CEO as final approval steps');
...
const getNextITApprovalStatus = (currentLevel, totalLevels, approvalChain = []) => {
  ...
  if (approvalChain && approvalChain.length > 0) {
    ...
    if (nextStep) {
      if (nextStep.approver.role === 'CEO - Final Authority') return 'pending_ceo';
      if (nextStep.approver.role === 'IT Department - Final Approval') return 'pending_it_approval';
    }
  }
  ...
  return statusMap[nextLevel] || 'pending_ceo';
};
...
const canUserApproveITRequest = (user, approvalStep) => {
  ...
  if (userEmail === 'tom@gratoengineering.com') {
    return approvalStep.approver.role === 'CEO - Final Authority';
  }
...
const getUserITApprovalLevel = (userRole, userEmail) => {
  ...
  if (email === 'tom@gratoengineering.com') {
    return 5;
  }
...
/**
 * Validate IT approval chain — CEO must be final step
 */
const validateITApprovalChain = (approvalChain) => {
  ...
  // CEO must be the last step
  const lastStep = approvalChain[approvalChain.length - 1];
  if (lastStep.approver.role !== 'CEO - Final Authority') {
    return { valid: false, error: 'CEO must be the final approver' };
  }
