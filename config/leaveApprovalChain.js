...
/**
 * Get Leave Approval Chain with STRICT 5-level hierarchy:
 * Level 1: Immediate Supervisor
 * Level 2: Department Head
 * Level 3: Head of Business (President)
 * Level 4: HR Department (Final Compliance)
...
const getLeaveApprovalChain = (employeeName, department) => {
  ...
  if (hrDepartment) {
    if (!seenEmails.has(hrDepartment.head.email)) {
      ...
      console.log(`✓ Level ${finalLevel}: HR Department (Final Compliance) - ${hrDepartment.head.email}`);
    }
  }

  // LEVEL 5: CEO — absolute final
  const ceEmail = 'tom@gratoengineering.com';
  if (!seenEmails.has(ceEmail)) {
    const ceLevel = chain.length + 1;
    chain.push({
      level: ceLevel,
      approver: {
        name:       'Mr. Tom',
    ...
    seenEmails.add(ceEmail);
    console.log(`✓ Level ${ceLevel}: Mr. Tom (CEO - Final Authority)`);
  }
