// ═══════════════════════════════════════════════════════════════════════════
// FILE: config/cashRequestApprovalChain.js
// VERSION: 2.2 — Conditional HR (missions only) + Conditional CEO (threshold)
// ═══════════════════════════════════════════════════════════════════════════
...
/**
 * ✅ VERSION 2.2: Get cash request approval chain with:
 *   - CONDITIONAL HR   → included only for mission-type requests
 *   - CONDITIONAL CEO  → included only when amount ≥ 100,000 XAF
 *
 * Mission requests  (amount < 100k):  Supervisor → Dept Head → HR → Finance → HOB          (5 levels)
 * Mission requests  (amount ≥ 100k):  Supervisor → Dept Head → HR → Finance → HOB → CEO    (6 levels)
 * Other requests    (amount < 100k):  Supervisor → Dept Head → Finance → HOB               (4 levels)
 * Other requests    (amount ≥ 100k):  Supervisor → Dept Head → Finance → HOB → CEO         (5 levels)
 *
 ...
 * @param {number|null} amount        - Request amount in XAF (null → CEO required as precaution)
 * @returns {Array} Approval chain steps
 */
const getCashRequestApprovalChain = (employeeEmail, requestType, amount = null) => {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`=== BUILDING CASH REQUEST APPROVAL CHAIN (V2.2) ===`);
  console.log(`${'='.repeat(60)}`);
  console.log(`🔹 Employee Email: ${employeeEmail}`);
  console.log(`🔹 Request Type:   ${requestType}`);
  console.log(`🔹 Amount (XAF):   ${amount !== null ? Number(amount).toLocaleString() : 'NOT PROVIDED'}`);
  console.log(`🔹 Version:        2.2 (Conditional HR + Conditional CEO)`);
  console.log(`🔹 Timestamp:      ${new Date().toISOString()}`);

  // ── Input validation ──────────────────────────────────────────────────────
  if (!employeeEmail || typeof employeeEmail !== 'string') {
    console.error('❌ Invalid employee email provided');
    return getFallbackApprovalChain(requestType, amount);
  }

  console.log(`✓ Input validation passed`);

  // ── Mission-request flag ──────────────────────────────────────────────────
  ...
  // ── CEO threshold check ───────────────────────────────────────────────────
  const ceoCheck = requiresCEOApproval('cash_request', amount);
  console.log(`✓ CEO Step:        ${ceoCheck.required ? 'REQUIRED' : 'SKIPPED'} — ${ceoCheck.reason}`);

  // ── STEP 1: Get base chain from org structure ─────────────────────────────
  ...
  if (!baseApprovalChain || baseApprovalChain.length === 0) {
    console.warn(`⚠️  No approval chain found for ${employeeEmail} — using fallback`);
    return getFallbackApprovalChain(requestType, amount);
  }
  ...
  // CEO comes from ceoApprovalConfig — single source of truth
  // CEO object: { name, email, role, department }
  ...
  console.log(`   CEO (Final):     ${CEO.name} — ${ceoCheck.required ? 'WILL BE ADDED' : 'WILL BE SKIPPED'}`);

  // ── STEP 3: Build chain ───────────────────────────────────────────────────
  console.log(`\n--- STEP 3: Building Approval Chain ---`);

  let processedChain = [];
  let currentLevel   = 1;
  const seenEmails   = new Set();

  // Strip HOB from base chain (we add it explicitly as penultimate)
  const hobEmailLower = HEAD_OF_BUSINESS.email.toLowerCase();
  const baseChainWithoutHOB = baseApprovalChain.filter(step => {
    const emailLower = String(step.approver?.email || '').trim().toLowerCase();
    if (emailLower === hobEmailLower) {
      console.log(`  🎯 Found HOB in base chain — will re-add as penultimate approver`);
      return false;
    }
    return true;
  });

  // ── 3a: Base chain (Supervisor → Dept Head, excluding HOB) ───────────────
  console.log(`\n  Adding Base Chain (Supervisor → Dept Head):`);
  baseChainWithoutHOB.forEach((step) => {
  ...
  // ── 3b: HR — only for mission requests ───────────────────────────────────
  if (isMissionRequest) {
  ...
  // ── 3d: Head of Business (penultimate) ───────────────────────────────────
  console.log(`\n  Adding Head of Business (Kelvin):`);
  const alreadyHasHOB = processedChain.some(
    step => String(step.approver?.email || '').trim().toLowerCase() === hobEmailLower
  );
  ...
  if (!alreadyHasHOB) {
    ...
    // Force-add HOB even if it slipped in via base chain (keeps penultimate slot)
    processedChain.push({
      level:        processedChain.length + 1,
      approver: {
        name:       HEAD_OF_BUSINESS.name,
    ...
    console.log(`  ⚠️  Forced Head of Business at level ${processedChain.length}`);
  }

  // ── 3e: CEO — conditional on amount threshold ─────────────────────────────
  console.log(`\n  CEO step: ${ceoCheck.required ? 'REQUIRED' : 'SKIPPED'} — ${ceoCheck.reason}`);

  if (ceoCheck.required) {
    const ceoFinalEmailLower = CEO.email.toLowerCase();
  ...
  console.log(`✅ CEO: ${ceoCheck.required ? 'INCLUDED' : 'EXCLUDED'} — ${ceoCheck.reason}`);

  const chainSummary = processedChain
...
/**
 * Map a role string from org structure to the display role used in cash chains.
 */
const mapRoleForCashApproval = (structureRole, level, email = '') => {
  ...
  if (emailLow === 'bruiline.tsitoh@gratoglobal.com')    return 'HR Head';
  if (emailLow === 'ranibellmambo@gratoengineering.com') return 'Finance Officer';
  if (emailLow === 'kelvin.eyong@gratoglobal.com')       return 'Head of Business';
  if (emailLow === 'tom@gratoengineering.com')           return 'CEO - Final Authority';

  if (roleLower.includes('finance'))                                          return 'Finance Officer';
  if (roleLower.includes('president') || roleLower === 'head of business')   return 'Head of Business';
  if (roleLower.includes('head') || roleLower.includes('director'))          return 'Departmental Head';
  if (roleLower.includes('supervisor') || roleLower.includes('manager') || roleLower.includes('coordinator')) return 'Supervisor';

  // Level-based fallback
  if (level === 1) return 'Supervisor';
  if (level === 2) return 'Departmental Head';
  return 'Approver';
};


/**
 * ✅ V2.2 Fallback chain — conditional HR (missions) + conditional CEO (threshold).
 *
 * @param {string}      requestType
 * @param {number|null} amount       - XAF amount; forwarded to requiresCEOApproval()
 */
const getFallbackApprovalChain = (requestType, amount = null) => {
  console.warn('\n⚠️⚠️⚠️  USING FALLBACK APPROVAL CHAIN (V2.2) ⚠️⚠️⚠️');
  console.warn('Employee not found in department structure.');
  console.warn('Add this employee to config/departmentStructure.js\n');
  ...
  if (ceoCheck.required) {
    ...
    console.log(`✅ Added CEO at level ${nextLevel} — ${ceoCheck.reason}`);
  } else {
    console.log(`⏭️  Skipped CEO — ${ceoCheck.reason}`);
  }

  console.log(`\n✅ Fallback chain created with ${fallbackChain.length} levels`);

  let structure = 'Supervisor → Dept Head';
  if (isMissionRequest) structure += ' → HR';
  structure += ' → Finance → HOB';
  if (ceoCheck.required)  structure += ' → CEO';
...
/**
 * Determine the next workflow status string after a given level is approved.
 */
...
/**
 * Validate the built approval chain.
 * Accepts CEO or Head of Business as a valid final approver.
 */
