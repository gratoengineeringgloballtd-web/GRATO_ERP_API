...
const processSupervisorDecision = async (req, res) => {
  try {
    ...
    if (requisition.status === 'justification_pending_supervisor') {
      ...
      requisition.justification.supervisorReview = { decision, comments, reviewedBy: req.user.userId, reviewedDate: new Date() };
...
const getDashboardStats = async (req, res) => {
  try {
    ...
    else if (role === 'supply_chain') filter.status = { $in: ['pending_supply_chain_review', 'supply_chain_approved', 'in_procurement', 'procurement_complete'] };
    else if (role === 'finance') filter.status = { $in: ['pending_finance_verification', 'pending_supply_chain_review', 'approved', 'in_procurement', 'procurement_complete', 'delivered'] };

    const [totalCount, pendingCount, approvedCount, rejectedCount, inProcurementCount, completedCount, recentRequisitions, monthlyStats] = await Promise.all([
      PurchaseRequisition.countDocuments(filter),
      PurchaseRequisition.countDocuments({ ...filter, status: { $in: ['pending_supervisor', 'pending_finance_verification', 'pending_supply_chain_review'] } }),
      PurchaseRequisition.countDocuments({ ...filter, status: { $in: ['approved', 'supply_chain_approved'] } }),
      PurchaseRequisition.countDocuments({ ...filter, status: { $in: ['rejected', 'supply_chain_rejected'] } }),
      PurchaseRequisition.countDocuments({ ...filter, status: 'in_procurement' }),
      PurchaseRequisition.countDocuments({ ...filter, status: { $in: ['procurement_complete', 'delivered'] } }),
    ...
    let stats = {
      summary: { total: totalCount, pending: pendingCount, approved: approvedCount, rejected: rejectedCount, inProcurement: inProcurementCount, completed: completedCount },
...
const getCategoryAnalytics = async (req, res) => {
  try {
    ...
    const analytics = await PurchaseRequisition.aggregate([
      ...
      { $group: { _id: '$itemCategory', count: { $sum: 1 }, totalBudget: { $sum: '$budgetXAF' }, avgBudget: { $avg: '$budgetXAF' }, approvedCount: { $sum: { $cond: [{ $in: ['$status', ['approved', 'in_procurement', 'procurement_complete', 'delivered']] }, 1, 0] } } } },
...
const getVendorPerformance = async (req, res) => {
  try {
    const mockVendorData = [
      { name: 'TechSolutions Cameroon', totalOrders: 28, onTimeDelivery: 95, qualityRating: 4.5, totalSpend: 15000000 },
      { name: 'Office Supplies Plus', totalOrders: 45, onTimeDelivery: 90, qualityRating: 4.2, totalSpend: 8500000 }
    ];
    res.json({ success: true, data: mockVendorData, message: 'Vendor performance data (mock)' });
  } catch (error) {
...
const getProcurementPlanningData = async (req, res) => {
  try {
    ...
    const [upcomingRequisitions, procurementPipeline, budgetUtilization] = await Promise.all([
      PurchaseRequisition.find({ status: { $in: ['approved', 'in_procurement'] }, expectedDate: { $gte: new Date(), $lte: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) } }).populate('employee', 'fullName department').sort({ expectedDate: 1 }).limit(20),
      PurchaseRequisition.aggregate([{ $match: { status: { $in: ['approved', 'in_procurement', 'procurement_complete'] } } }, { $group: { _id: '$status', count: { $sum: 1 }, totalValue: { $sum: '$budgetXAF' } } }]),
      PurchaseRequisition.aggregate([{ $match: { status: { $in: ['approved', 'in_procurement', 'procurement_complete', 'delivered'] }, createdAt: { $gte: new Date(new Date().getFullYear(), new Date().getMonth(), 1) } } }, { $group: { _id: '$itemCategory', allocated: { $sum: '$budgetXAF' }, spent: { $sum: '$procurementDetails.finalCost' }, count: { $sum: 1 } } }])
    ]);
...
const getRequisitionsByRole = async (req, res) => {
  try {
    ...
    switch (user.role) {
      ...
      case 'finance': query = { ...baseFilter, $or: [{ status: 'pending_finance' }, { status: 'approved' }, { status: 'delivered' }] }; break;
...
const getFinanceDashboardData = async (req, res) => {
  try {
    ...
    const financeRequisitions = await PurchaseRequisition.find({ status: { $in: ['pending_finance_verification', 'pending_supply_chain_review', 'pending_buyer_assignment', 'pending_head_approval', 'approved', 'in_procurement', 'procurement_complete'] } }).populate('employee', 'fullName email department').sort({ createdAt: -1 });
...
const getPurchaseRequisitionDashboardStats = async (req, res) => {
  try {
    ...
    const stats = {
      pending: requisitions.filter(r => ['draft', 'pending_supervisor', 'pending_finance_verification', 'pending_supply_chain_review', 'pending_buyer_assignment', 'pending_head_approval', 'pending_ceo_approval'].includes(r.status)).length,
      total: requisitions.length,
      ...(userRole === 'buyer' && {
        inProgress: requisitions.filter(r => r.status === 'in_procurement' && r.supplyChainReview?.assignedBuyer?.equals(userId)).length,
        quotesReceived: requisitions.filter(r => r.status === 'procurement_complete' && r.supplyChainReview?.assignedBuyer?.equals(userId)).length,
        completed: requisitions.filter(r => ['procurement_complete', 'delivered'].includes(r.status) && r.supplyChainReview?.assignedBuyer?.equals(userId)).length
      })
    };
...
const processDisbursement = async (req, res) => {
  try {
    ...
    if (requisition.financeVerification?.budgetCode) {
      const BudgetCode = require('../models/BudgetCode');
      const budgetCode = await BudgetCode.findOne({ code: requisition.financeVerification.budgetCode });
      if (budgetCode) {
        if (!budgetCode.allocations) budgetCode.allocations = [];
        budgetCode.allocations.push({ requisitionId: requisition._id, amount: disbursementAmount, allocatedBy: req.user.userId, allocatedDate: new Date(), status: 'disbursed' });
